...
- **QR Ticket + NFC Gate Registration:** Gate staff scan attendee QR tickets (HMAC-signed tokens), system validates ticket for today's event day (timezone-aware), shows attendee profile, then links NFC bracelet to attendee — all in one atomic DB transaction. Supports multi-day passes/abonos. Schema: `ticket_checkins` table with unique constraint on `(event_id, ticket_id, event_day_index)`. API routes: `POST /api/gate/validate-ticket`, `POST /api/gate/ticket-checkin`, `GET /api/gate/checkin-history`.
