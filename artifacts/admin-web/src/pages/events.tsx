    ...
    <div className="space-y-4">
      ...
      <div className="border-t pt-3 space-y-3">
        <div className="space-y-1">
          ...
          <Select
            value={form.promoterCompanyId || "__none__"}
            onValueChange={(v) => setForm((f) => ({ ...f, promoterCompanyId: v === "__none__" ? "" : v }))}
          >
            <SelectTrigger data-testid="select-promoter-company">
              <SelectValue placeholder={t("events.selectPromoterCompany")} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="__none__">{t("events.noPromoterCompany")}</SelectItem>
              {promoterCompanies.map((pc) => (
                <SelectItem key={pc.id} value={pc.id}>{pc.companyName}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
