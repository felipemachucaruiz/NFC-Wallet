...
export function DateTimePicker({ value, onChange, placeholder, className, "data-testid": testId }: DateTimePickerProps) {
  ...
  const currentTime = timePart || defaultTime;
  ...
  const displayText = validDate
    ? `${format(validDate, "PPP", { locale })} ${currentTime}`
    : null;
  ...
  const handleDayClick = (day: Date) => {
    emitValue(format(day, "yyyy-MM-dd"), currentTime);
    setView("days");
  };
      ...
      <PopoverContent className="w-[264px] p-4" align="start">
        ...
        <div className="mt-3 pt-3 border-t flex items-center gap-2">
          ...
          <input
            ...
            className="flex-1 bg-transparent text-sm border border-input rounded-md px-2 py-1 focus:outline-none focus:ring-1 focus:ring-ring text-foreground"
          />
        </div>
      </PopoverContent>
    </Popover>
