    ...
    return (
      <View style={[styles.center, { backgroundColor: C.background }]}>
        ...
        <Text style={[styles.successTitle, { color: C.text }]}>
          {t("attendeeBlock.blockedTitle")}
        </Text>
        <Text style={[styles.successSubtitle, { color: C.textSecondary }]}>
          {t("attendeeBlock.blockedMessage")}
        </Text>
        <Button
          title={t("common.back")}
          onPress={() => router.back()}
  ...
  return (
    ...
    >
      <View style={styles.headerRow}>
        ...
        <Text style={[styles.pageTitle, { color: C.text }]}>{t("attendeeBlock.title")}</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      <Card>
        <View style={styles.warningBox}>
          <Feather name="alert-triangle" size={20} color={C.warning ?? "#F59E0B"} />
          <Text style={[styles.warningTitle, { color: C.text }]}>
            {t("attendeeBlock.warningTitle")}
          </Text>
        </View>
        <View style={styles.warningList}>
          {[
            t("attendeeBlock.warning1"),
            t("attendeeBlock.warning2"),
            t("attendeeBlock.warning3"),
          ].map((w, i) => (
            <View key={i} style={styles.warningItem}>
              <View style={[styles.dot, { backgroundColor: C.warning ?? "#F59E0B" }]} />
      ...
      <Card>
        <View style={styles.unlockInfo}>
          ...
          <Text style={[styles.unlockText, { color: C.textSecondary }]}>
            {t("attendeeBlock.unlockInfo")}
          </Text>
        </View>
      </Card>

      <Button
        title={t("attendeeBlock.blockBtn")}
        onPress={handleBlock}
        variant="danger"
        size="lg"
        fullWidth
